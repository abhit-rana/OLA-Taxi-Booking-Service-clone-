...
<body>
    ...
    <?php
        ...
        if(isset($_POST['src']) && isset($_POST['dest'])){
            ...
            $dest = $_POST['dest'];
            if(empty($src)){
                header("Location: index.php?error=Source is required to search for the cabs!");
                exit();
            }else if(empty($dest)){
                header("Location: index.php?error=Destination is required is required to search for the cabs!");
                exit();
            }else{
                $flag = true;
                $_SESSION['src'] = $src;
                ...
                $query = "select c.carName as name, d.did, c.chargePerKm as charge, cc.catname as cat from locationstatus ls, driver d, car c, carcategories cc where ls.did=d.did and CONCAT(ls.block, ls.city, ls.state) LIKE '$src' and c.catId=cc.catId and c.did= d.did;";
                ...
                //AT PRESENT DONT KNOW HOW TO SLEECT THE OPTION FROM SELECTION OF DIFFERENT BUTTONS
                // therefore Hardcoding the value at the presetn
                    ...
                    while ($row=mysqli_fetch_array($result)) {
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 col-lg-4">
                                <div class="card-mb-3">
                                    <img src="img/sedan1.jpg" alt="" class="card-img-top">
                                    <div class="card-body">
                                        <h4 class="card-title"> <?php echo $row['name'] . "(" . $row['cat'] .")"; ?> </h4>
                                        <?php 
                                            $_SESSION['carname']=$row['name'] . "(" . $row['cat'] .")"; 
