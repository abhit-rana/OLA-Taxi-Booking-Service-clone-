...
//validate username
	...
	$query = "SELECT * from driver where dusername = '$dusername'";
	$result = @mysqli_query($conn,$query)
	Or die ("<p>Unable to query the table.</p>"."<p>Error code ".
	mysqli_errno($conn). ": ".mysqli_error($conn)). "</p>";
			$row = mysqli_fetch_row($result);
			if ($dusername !== $row[4]) { // check username whether the member exists
			echo "The member does not existed. Please register.<br>";
			}
			elseif ($dlicenseno == $row[7]) { //check whether the password is matched 
